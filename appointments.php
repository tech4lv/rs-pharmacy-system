    ...
    if ($action === 'save_appointment') {
        ...
        if ($patientId && $apptDate && $apptTime) {
            if ($editId > 0) {
                ...
                $stmt->bind_param('isssississi', $patientId, $doctorName, $apptDate, $apptTime, $duration, $purpose, $serviceType, $status, $notes, $editId);
                $stmt->execute();
                ...
                $stmt->bind_param('isssississs', $patientId, $doctorName, $apptDate, $apptTime, $duration, $purpose, $serviceType, $status, $notes);
                $stmt->execute();
                logAudit($user['id'], "Appointment booked for patient #$patientId on $apptDate", 'CREATE', 'appointments', $db->insert_id);
                $message = 'Appointment booked!'; $messageType = 'success';
            }
        } else { $message = 'Please fill in all required fields.'; $messageType = 'danger'; }
...
$appts = [];
$res = $db->query("SELECT a.*, CONCAT(p.first_name,' ',p.last_name) as patient_name FROM appointments a LEFT JOIN patients p ON a.patient_id=p.id WHERE MONTH(a.appointment_date)=$month AND YEAR(a.appointment_date)=$year ORDER BY a.appointment_date, a.appointment_time");
while ($row = $res->fetch_assoc()) $appts[] = $row;

// Group by date
$apptsByDate = [];
foreach ($appts as $a) { $apptsByDate[$a['appointment_date']][] = $a; }
    ...
    <div class="main-content">
        ...
        <div class="page-content">
            <div class="page-header">
                <div><div class="page-title">Appointment Calendar</div><div class="page-subtitle">Schedule and manage patient appointments</div></div>
    ...
    document.getElementById('viewApptBody').innerHTML = `
        <div style="background:var(--gray-ultra);border-radius:var(--radius-sm);padding:16px;margin-bottom:16px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:13px;">
                ...
                <div><span style="color:var(--gray);">Doctor: </span><strong>${a.doctor_name || '—'}</strong></div>
                ...
                <div><span style="color:var(--gray);">Purpose: </span><strong>${a.purpose || '—'}</strong></div>
            </div>
        </div>
    `;
